...
function FixingCap(M,Type, ref_PV_cap, ref_BT_cap)
    if Household_types[Household_types[!, "Type"] .== Type, "PV"][1] == 0
        fix(M[:C_PV], 0; force=true)
    else
        fix(M[:C_PV], ref_PV_cap; force=true)
    end
    if Household_types[Household_types[!, "Type"] .== Type, "BT"][1] == 0
        fix(M[:C_BT], 0; force=true)
    else
        fix(M[:C_BT], ref_BT_cap; force=true)
    end
end
...
function DefineConstraints(M, scheme)
    if scheme == "new"
        @objective(M, Min, sum(Scalars["CRF"]*PV_par["Capital_cost"]*M[:C_PV]
            ...
            + (Network_tariffs["Tax"] * sum(M[:g_im_load][t,y] + M[:p_PV_load][t,y] + M[:g_im_bat][t,y] + M[:p_PV_bat][t,y] - M[:b_dh_ex][t,y] for t in T for y in Y)))
    ...
    @constraint(M, Balance[t in T, y in Y], M[:g_im_load][t,y] + M[:b_dh_load][t,y] + M[:p_PV_load][t,y] - Demand[t,y] == 0)

    # SOC regular balance when the hours set is not 1
    @constraint(M, SOC[t in T, y in Y; t>1], M[:b_st][t,y] == M[:b_st][t-1,y] - M[:b_dh][t,y]/Battery_par["Discharging_eff"] + M[:b_ch][t,y]*Battery_par["Charging_eff"])

    # SOC balance for the first hour and NOT first year
    @constraint(M, SOC_LastT[t in T, y in Y; t == 1 && y!=1], M[:b_st][t,y] == M[:b_st][last(T),y-1] - M[:b_dh][t,y]/Battery_par["Discharging_eff"] + M[:b_ch][t,y]*Battery_par["Charging_eff"])

    # SOC balance for the first hour and first year
    @constraint(M, SOC_First[t in T, y in Y; t==1 && y==1], M[:b_st][t,y] == M[:C_BT] - M[:b_dh][t,y]/Battery_par["Discharging_eff"] + M[:b_ch][t,y]*Battery_par["Charging_eff"] )

    # Limit on the maximum charge state of charge of the battery
    @constraint(M, SOC_lim_up[t in T, y in Y], M[:b_st][t,y] <= Battery_par["Max_charge"]*M[:C_BT])

    # Limit on the maximum hourly charging
    @constraint(M, Charge_limit[t in T, y in Y], M[:b_ch][t,y] <= Battery_par["Charging_lim"]*M[:C_BT])

    # Limit on the maximum hourly discharging
    @constraint(M, Discharge_limit[t in T, y in Y], M[:b_ch][t,y] <= Battery_par["Discharging_lim"]*M[:C_BT])

    # Limit on the minimum battery state of charge (depth of discharge)
    @constraint(M, SOC_lim_down[t in T, y in Y], M[:b_st][t,y] >= (1-Battery_par["Depth_of_discharge"])*M[:C_BT])

    # Limit on the amount of hourly exported electricity
    @constraint(M, grid_ex_lim[t in T, y in Y], M[:g_ex][t,y] <= Grid_par["Ex_lim"])

    # Limit on the amount of hourly imported electricity
    @constraint(M, grid_im_lim[t in T, y in Y], M[:g_im][t,y] <= Grid_par["Im_lim"])

    # Balance of the imported energy
    @constraint(M, grid_im_def[t in T, y in Y], M[:g_im][t,y] == M[:g_im_load][t,y] + M[:g_im_bat][t,y])

    # Balance of the exported energy
    @constraint(M, grid_ex_def[t in T, y in Y], M[:g_ex][t,y] == M[:p_PV_ex][t,y] + M[:b_dh_ex][t,y])

    # Balance of the charging energy
    @constraint(M, bat_ch_def[t in T, y in Y], M[:b_ch][t,y] == M[:p_PV_bat][t,y] + M[:g_im_bat][t,y])

    # Balance of the discharging energy
    @constraint(M, bat_dh_def[t in T, y in Y], M[:b_dh][t,y] == M[:b_dh_ex][t,y] + M[:b_dh_load][t,y])

    # Definition of the PV array production
    @constraint(M, PV_prod_def[t in T, y in Y], M[:C_PV]*PV_CF[t,y] == M[:p_PV][t,y])

    # Balance of the PV energy
    @constraint(M, PV_prod_bal[t in T, y in Y], M[:p_PV][t,y] == M[:p_PV_bat][t,y] + M[:p_PV_ex][t,y] + M[:p_PV_load][t,y])
...
function ExportResults(M, filename)
    ...
    XLSX.writetable(filename,
                                b_st=( collect(DataFrames.eachcol(ExportVariable(M[:b_st],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:b_st],[T,Y],["T","Y"]))),

                                b_dh=( collect(DataFrames.eachcol(ExportVariable(M[:b_dh],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:b_dh],[T,Y],["T","Y"]))),

                                b_dh_load=( collect(DataFrames.eachcol(ExportVariable(M[:b_dh_load],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:b_dh_load],[T,Y],["T","Y"]))),

                                b_dh_ex=( collect(DataFrames.eachcol(ExportVariable(M[:b_dh_ex],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:b_dh_ex],[T,Y],["T","Y"]))),

                                b_ch=( collect(DataFrames.eachcol(ExportVariable(M[:b_ch],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:b_ch],[T,Y],["T","Y"]))),

                                p_PV=( collect(DataFrames.eachcol(ExportVariable(M[:p_PV],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:p_PV],[T,Y],["T","Y"]))),

                                p_PV_load=( collect(DataFrames.eachcol(ExportVariable(M[:p_PV_load],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:p_PV_load],[T,Y],["T","Y"]))),

                                p_PV_bat=( collect(DataFrames.eachcol(ExportVariable(M[:p_PV_bat],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:p_PV_bat],[T,Y],["T","Y"]))),

                                g_ex=( collect(DataFrames.eachcol(ExportVariable(M[:g_ex],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:g_ex],[T,Y],["T","Y"]))),

                                g_im=( collect(DataFrames.eachcol(ExportVariable(M[:g_im],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:g_im],[T,Y],["T","Y"]))),

                                g_im_load=( collect(DataFrames.eachcol(ExportVariable(M[:g_im_load],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:g_im_load],[T,Y],["T","Y"]))),

                                g_im_bat=( collect(DataFrames.eachcol(ExportVariable(M[:g_im_bat],[T,Y],["T","Y"]))), DataFrames.names(ExportVariable(M[:g_im_bat],[T,Y],["T","Y"]))),
                                )
end
